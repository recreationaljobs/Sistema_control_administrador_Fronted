...
import {
  ...
  getEstadosAdelanto,
  getRecibo,
  getResumenAdelantosPorConductor,
  getSucursales,
  updateAdelanto,
...
export const useAdelantos =
  () => {
     ...
     * individuales de adelantos y
     * abonos.
     */
    const cargarAdelantos =
      async ({
        mostrarCarga = true,
      } = {}) => {
        try {
    ...
    /*
     * Carga una fila por conductor
     * con su total adelantado,
     * abonado y saldo pendiente.
     */
    const cargarResumenConductores =
      async ({
        mostrarCarga = true,
        estadoSaldo =
          filtroEstadoSaldo,
        ...
        try {
          ...
          const data =
            await getResumenAdelantosPorConductor(
              {
                estado_saldo:
                  estadoSaldo ||
                  "pendiente",
          ...
          const mensaje =
            obtenerMensajeError(
              requestError,
              "No se pudo cargar el resumen por conductor."
            );
    ...
    const guardarAdelanto =
      async (form) => {
        ...
        try {
          ...
          /*
           * Actualiza movimientos y
           * resumen sin ocultar la
           * interfaz.
           */
          await Promise.all([
            cargarAdelantos({
    ...
    /*
     * Totales generales de todos
     * los movimientos visibles para
     * el usuario.
     */
     ...
     * conductor.
     */
    const obtenerMovimientosConductor =
      (conductorId) => {
        if (!conductorId) {
          return [];
        }

        return adelantosConSaldo
          .filter(
            (movimiento) => {
              return (
                String(
    ...
    /*
     * Recarga el resumen cuando
     * cambia el filtro de saldo.
     */
    useEffect(() => {
      void cargarResumenConductores(
        {
          mostrarCarga:
            true,
