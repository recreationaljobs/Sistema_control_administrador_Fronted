...
const JornadaForm = ({
  ...
  const puedeEditarIngreso = esSuperAdmin || esAdminSucursal;
  const modoCierre = jornadaEditando?.modoFormulario === "cerrar";
  const modoEdicion = Boolean(jornadaEditando) && !modoCierre;
  const mostrarLiquidacion = puedeEditarIngreso && modoEdicion;
  const mostrarBusquedaConductor = !esTaxista && !modoEdicion;
  const mostrarKilometrajeInicial = !mostrarLiquidacion;
  const mostrarKilometrajeFinal =
    !mostrarLiquidacion && (modoCierre || modoEdicion);
  ...
  const conductoresConAsignacion = useMemo(() => {
    const idsConductoresAsignados = asignacionesActivas.map((item) =>
      String(item.conductor)
    );

    return conductores.filter((conductor) =>
      idsConductoresAsignados.includes(String(conductor.id))
    );
  }, [conductores, asignacionesActivas]);

  const conductorSeleccionado = useMemo(() => {
    return conductores.find(
      (item) => String(item.id) === String(form.conductor)
    );
  ...
  const asignacionSeleccionada = useMemo(() => {
    ...
    return (
      asignacionesActivas.find(
        (item) => String(item.conductor) === String(form.conductor)
      ) || null
    );
  }, [asignacionesActivas, esTaxista, form.conductor]);

  const vehiculoSeleccionado = useMemo(() => {
    const vehiculoId = form.vehiculo || asignacionSeleccionada?.vehiculo;
    ...
    return (
      vehiculos.find((item) => String(item.id) === String(vehiculoId)) || null
    );
  }, [vehiculos, form.vehiculo, asignacionSeleccionada]);

  const conductoresFiltrados = useMemo(() => {
    const value = busquedaConductor.trim().toLowerCase();

    if (!value) {
      return conductoresConAsignacion.slice(0, 8);
    }

    return conductoresConAsignacion
      .filter((conductor) => {
        const nombre = obtenerNombreConductor(conductor).toLowerCase();
        const cedula = conductor.cedula?.toLowerCase() || "";

        return nombre.includes(value) || cedula.includes(value);
      })
      .slice(0, 8);
  }, [busquedaConductor, conductoresConAsignacion]);
  ...
  const nombreConductorMostrado = conductorSeleccionado
    ? obtenerNombreConductor(conductorSeleccionado)
    : jornadaEditando?.conductor_nombre ||
      (esTaxista ? "Conductor autenticado" : "No seleccionado");

  const vehiculoTexto = vehiculoSeleccionado
    ? `${vehiculoSeleccionado.numero} - ${vehiculoSeleccionado.placa}`
    : jornadaEditando?.vehiculo_numero || jornadaEditando?.vehiculo_placa
  ...
  useEffect(() => {
    if (jornadaEditando) {
      const conductor = conductores.find(
        (item) => String(item.id) === String(jornadaEditando.conductor)
      );

      const tipoCobroDetectado =
        jornadaEditando.tipo_cobro ||
        (Number(jornadaEditando.monto_alquiler || 0) > 0
          ? "alquiler"
          : "porcentaje");
      ...
      setForm({
        ...
        tipo_cobro: tipoCobroDetectado,
        ingreso_bruto: String(jornadaEditando.ingreso_bruto ?? ""),
        monto_alquiler: String(jornadaEditando.monto_alquiler ?? ""),
        porcentaje_pago_conductor: String(
          jornadaEditando.porcentaje_pago_conductor ?? "30"
        ),
  ...
  useEffect(() => {
    if (esTaxista && asignacionSeleccionada) {
      setForm((prev) => ({
        ...prev,
        conductor: asignacionSeleccionada.conductor
          ? String(asignacionSeleccionada.conductor)
          : "",
        vehiculo: asignacionSeleccionada.vehiculo
          ? String(asignacionSeleccionada.vehiculo)
          : "",
      }));
    }
  }, [esTaxista, asignacionSeleccionada]);
  ...
  const seleccionarConductor = (conductor) => {
    const asignacion = asignacionesActivas.find(
      (item) => String(item.conductor) === String(conductor.id)
    );

    setForm((prev) => ({
      ...prev,
      conductor: String(conductor.id),
      vehiculo: asignacion?.vehiculo ? String(asignacion.vehiculo) : "",
    }));
    ...
    setForm((prev) => {
      if (name === "tipo_cobro") {
        return {
          ...prev,
          tipo_cobro: value,
          ingreso_bruto: value === "alquiler" ? "" : prev.ingreso_bruto,
          monto_alquiler: value === "porcentaje" ? "" : prev.monto_alquiler,
          porcentaje_pago_conductor:
            value === "alquiler" ? "0" : prev.porcentaje_pago_conductor || "30",
        };
      }
  ...
  const handleSubmit = (event) => {
    ...
    if (!esTaxista && !form.conductor) {
      setFormError("Debes buscar y seleccionar un conductor.");
      return;
    }
    ...
    const kmFinal =
      form.kilometraje_final !== "" && form.kilometraje_final !== null
        ? Number(form.kilometraje_final)
        : null;

    const tipoCobro = form.tipo_cobro || "porcentaje";
    const ingresoBruto = Number(form.ingreso_bruto || 0);
    const montoAlquiler = Number(form.monto_alquiler || 0);
    const porcentajePago =
      tipoCobro === "alquiler"
        ? 0
        : Number(form.porcentaje_pago_conductor || 30);
    ...
    if (!mostrarLiquidacion) {
      ...
      if (modoCierre || modoEdicion) {
        if (kmFinal === null || Number.isNaN(kmFinal) || kmFinal < 0) {
          setFormError("Debes ingresar un kilometraje final válido.");
          return;
        }
    ...
    if (mostrarLiquidacion) {
      ...
      if (tipoCobro === "alquiler") {
        if (Number.isNaN(montoAlquiler) || montoAlquiler < 0) {
          setFormError("El monto de alquiler debe ser un número válido.");
          return;
        }
      }
      ...
      onSave({
        ...
        kilometraje_inicial: Number(form.kilometraje_inicial || 0),
        kilometraje_final: kmFinal,
        tipo_cobro: tipoCobro,
        ingreso_bruto: tipoCobro === "porcentaje" ? ingresoBruto : 0,
        monto_alquiler: tipoCobro === "alquiler" ? montoAlquiler : 0,
        porcentaje_pago_conductor: porcentajePago,
    ...
    <form onSubmit={handleSubmit} className="px-6 py-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-slate-500">
              {modoCierre
                ? "Ingresa el kilometraje final al terminar la jornada."
                : mostrarLiquidacion
          ...
          <div className="rounded-2xl bg-white px-4 py-3 shadow-sm">
            <p className="text-xs font-bold text-slate-500">Fecha interna</p>
      ...
      {esTaxista && (
        <div className="mb-5 rounded-2xl border border-purple-200 bg-purple-50 px-4 py-3 text-sm font-semibold text-purple-700">
          Solo puedes ingresar kilometraje. El ingreso bruto o alquiler será registrado por administración.
        </div>
      )}
            ...
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-slate-500">
                  Selecciona el conductor. El vehículo activo se cargará solo.
                </p>
              </div>
            </div>
              ...
              <div className="mt-3 max-h-64 overflow-y-auto rounded-2xl border border-slate-200 bg-white">
                {conductoresFiltrados.length > 0 ? (
                  ...
                  <div className="px-4 py-5 text-sm font-semibold text-red-500">
                    No hay conductores con asignación activa.
                  </div>
                )}
              </div>
            )}

            {!conductoresConAsignacion.length && !loadingCatalogos && (
              <p className="mt-3 text-sm font-semibold text-red-500">
                No hay conductores con asignación activa. Primero crea una asignación.
              </p>
            )}
          </section>
        )}
          ...
          <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm md:col-span-2">
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-slate-500">
                  El vehículo se obtiene automáticamente de la asignación activa.
                </p>
              </div>
            </div>
          ...
          {mostrarKilometrajeInicial && (
            <div
              className={`rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm ${
                !mostrarKilometrajeFinal ? "md:col-span-2" : ""
              }`}
            >
              <div className="mb-4 flex items-center gap-3">
            ...
            <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <p
                className={`mt-3 text-sm font-black ${
                  diferenciaKm < 0 ? "text-red-600" : "text-slate-700"
                }`}
              >
            ...
            <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm md:col-span-2">
              <div className="mb-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-slate-500">
                    Selecciona el tipo de cobro para calcular el resultado.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                {form.tipo_cobro === "porcentaje" && (
                  <div>
                    ...
                    <p className="mt-2 text-xs font-semibold text-slate-500">
                      El sistema calcula el pago del conductor según el porcentaje configurado.
                    </p>
                  </div>
                )}
                ...
                {form.tipo_cobro === "alquiler" && (
                  <div>
                    ...
                    <p className="mt-2 text-xs font-semibold text-slate-500">
                      En alquiler no se calcula pago al conductor. La ganancia del dueño se calcula con el monto de alquiler menos gastos.
                    </p>
                  </div>
                )}
              </div>
